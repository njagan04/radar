...
async def test_none_reply_text_persists_fallback_instead_of_500(
    ...
    with patch(
        "chat.service.chat_agent.run_chat_turn",
        new=AsyncMock(
            return_value=ChatTurnResult(kind="reply", reply_text=None, tool_calls=[])
        ),
    ):
        r = await chat_client.post(
            f"/chat/threads/{thread_id}/messages",
...
async def test_ad_hoc_thread_tool_call_audits_with_null_investigation_id(
    ...
    with (
        patch.object(rbac_module, "_TOOL_REGISTRY", {"fake_tool": fake_tool}),
        patch.object(
            rbac_module,
        ...
        async with chat_db_factory() as db:
            ...
            gateway = RBACGateway(
                db=db, investigation_id=None, thread_id=fake_thread_id
            )
...
@pytest.mark.asyncio
async def test_first_ever_failure_seed_message_says_no_prior_fix_not_seen_once(
    chat_client, chat_db_factory
):
    ...
    payload = {
        ...
        "error_detail": {
            "error_code": "9999",
            "message": "ErrorCode=SomeSpecificFailure,...",
        },
        "failure_count": 1,
    }
    body = json.dumps(payload).encode()
