...
from mcp_servers.adf.tool_search_tool import build_chat_tools, build_keyword_index, retrieve_relevant_tools
from db.models import RBACPermission
from mcp_servers.adf.schemas import SPECS

_STATE = {
    "investigation_id": None, "pipeline_name": "orders_pipeline",
    "project": "acme", "platform": "adf",
}
...
async def _seed_all_rbac_rows(db_factory, overrides: dict[str, bool] | None = None):
    ...
    consent_required_operations = {
        "create_resource", "update_resource_definition", "rollback_resource_definition",
        "back_resource_definition", "forward_resource_definition", "rerun_pipeline",
        "cancel_pipeline_run", "start_trigger", "stop_trigger", "rerun_trigger_run",
        "cancel_trigger_run", "start_integration_runtime",
    }
    async with db_factory() as db:
        for spec in SPECS:
            allowed = overrides.get(spec.name, True)
            db.add(RBACPermission(
                tool_name=spec.name, allowed=allowed,
                requires_consent=spec.registry_tool_name in consent_required_operations,
            ))
        await db.commit()


@pytest.mark.asyncio
async def test_disallowed_tool_never_appears_regardless_of_retrieval_score(chat_db_factory):
    await _seed_all_rbac_rows(chat_db_factory, overrides={"rollback_dataset_definition": False})
    ctx = WorkflowContext(db_factory=chat_db_factory, redis=None)

    tools = await build_chat_tools(
        _STATE, ctx, "please roll back the orders dataset to its previous version", user_id="alice@acme.com",
    )
    names = [t.name for t in tools]
    assert "rollback_dataset_definition" not in names
    assert "rollback_pipeline_definition" not in names
...
async def test_needs_approval_matches_registry_row_for_a_direct_tool(chat_db_factory):
    ...
    tools = await build_chat_tools(_STATE, ctx, "kill the pipeline run that's stuck", user_id="alice@acme.com")
    by_name = {t.name: t for t in tools}

    assert "cancel_pipeline_run" in by_name
    assert by_name["cancel_pipeline_run"].needs_approval is True  # direct entry, requires_consent=True

    assert "list_pipelines" in by_name  # always-include core tool
    assert by_name["list_pipelines"].needs_approval is False  # list_resources is read-only


@pytest.mark.asyncio
async def test_needs_approval_matches_registry_row_for_a_generic_derived_tool(chat_db_factory):
    await _seed_all_rbac_rows(chat_db_factory)
    ctx = WorkflowContext(db_factory=chat_db_factory, redis=None)
    ...
    assert "rollback_dataset_definition" in by_name
    assert by_name["rollback_dataset_definition"].needs_approval is True  # generic-derived, shares rollback_resource_definition's row


@pytest.mark.asyncio
async def test_rerun_pipeline_needs_approval_like_any_other_mutating_tool(chat_db_factory):
    ...
    tools = await build_chat_tools(_STATE, ctx, "please rerun the orders pipeline", user_id="alice@acme.com")
    by_name = {t.name: t for t in tools}
...
def test_trigger_a_run_phrasing_does_not_penalize_rerun_pipeline():
    ...
    selected = retrieve_relevant_tools(
        "please trigger a run of it", SPECS, index, top_k=6,
...
def test_semantic_scoring_surfaces_linked_service_tools_for_indirect_phrasing():
    ...
    selected = retrieve_relevant_tools(
        message, SPECS, index, top_k=6, always_include=always_include, message_embedding=message_embedding,
    )
    selected_names = {spec.name for spec in selected}
    assert "get_linked_service" in selected_names or "get_linked_service_definition_raw" in selected_names
