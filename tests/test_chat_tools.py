...
_TOOL_CTX = ToolContext(context=None, tool_name="record_diagnosis_outcome", tool_call_id="test", tool_arguments="{}")

_STATE = {
    "investigation_id": None, "thread_id": None, "pipeline_name": "orders_pipeline",
...
async def _seed_all_rbac_rows(db_factory):
    ...
    async with db_factory() as db:
        for registry_tool_name in {spec.registry_tool_name for spec in SPECS}:
            db.add(RBACPermission(tool_name=registry_tool_name, allowed=True, requires_consent=False))
        await db.commit()


@pytest.mark.asyncio
async def test_custom_lookup_tools_always_present_regardless_of_platform(chat_db_factory):
    ctx = WorkflowContext(db_factory=chat_db_factory, redis=None)

    tools = await build_tools_for_platform("unknown_future_platform", _STATE, ctx, "some question", user_id="alice@acme.com")
    names = [t.name for t in tools]
    assert names == ["check_known_fix", "record_diagnosis_outcome"]  # no platform tools, generic ones still present
...
async def test_custom_lookup_tools_present_alongside_adf_tools(chat_db_factory):
    ...
    tools = await build_tools_for_platform("adf", _STATE, ctx, "some unrelated question", user_id="alice@acme.com")
    names = [t.name for t in tools]
...
async def test_record_diagnosis_outcome_creates_then_updates_same_row(chat_db_factory):
    ctx = WorkflowContext(db_factory=chat_db_factory, redis=None)
    tools = await build_tools_for_platform("unknown_future_platform", _STATE, ctx, "q", user_id="alice@acme.com")
    record_tool = _record_tool(tools)

    first = json.loads(await record_tool.on_invoke_tool(_TOOL_CTX, json.dumps({
    ...
    second = json.loads(await record_tool.on_invoke_tool(_TOOL_CTX, json.dumps({
        "error_signature": "conn_timeout_sql01",
        "error_category": "timeout",
    ...
    assert rows[0].root_cause == "SQL01 connection pool exhausted"  # preserved from first call
    assert rows[0].fix_applied == "Increased pool size to 50"  # added by second call


@pytest.mark.asyncio
async def test_record_diagnosis_outcome_different_signature_creates_separate_row(chat_db_factory):
    ctx = WorkflowContext(db_factory=chat_db_factory, redis=None)
    tools = await build_tools_for_platform("unknown_future_platform", _STATE, ctx, "q", user_id="alice@acme.com")
    record_tool = _record_tool(tools)

    await record_tool.on_invoke_tool(_TOOL_CTX, json.dumps({
        "error_signature": "conn_timeout_sql01", "error_category": "timeout",
    }))
    await record_tool.on_invoke_tool(_TOOL_CTX, json.dumps({
        "error_signature": "bad_json_payload", "error_category": "data_quality",
    }))
    ...
    assert len(rows) == 2
    assert {r.error_signature for r in rows} == {"conn_timeout_sql01", "bad_json_payload"}
