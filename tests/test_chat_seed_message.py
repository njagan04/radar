...
def test_extract_error_code_from_leaf():
    assert (
        extract_error_code({"leaf": {"error_code": "UserErrorInvalidCredentials"}})
...
def test_extract_error_code_message_pattern_checked_in_leaf_too():
    error_detail = {
        "leaf": {"error_code": "2200", "message": "ErrorCode=SqlFailedToConnect,..."}
    }
...
def test_extract_error_code_falls_back_to_numeric_field_when_message_has_no_pattern():
    assert (
        extract_error_code({"error_code": "2200", "message": "no ErrorCode token here"})
        == "2200"
    )
...
def test_build_seed_message_with_matching_rca():
    rca = ProjectRCA(
        ...
        error_signature="s",
        error_category="c",
        fix_applied="restart the linked service",
        failure_count=3,
    )
