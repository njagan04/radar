"""
Unit tests for mcp_servers.adf.schemas (the 66-distinct-tool declarative spec table, one file
...
from mcp_servers.adf.tool_search_tool import build_keyword_index, retrieve_relevant_tools
from mcp_servers.adf.schemas import SPECS
from mcp_servers.adf.tools import TOOL_REGISTRY

_RBAC_SEED_MIGRATION = (
    Path(__file__).parent.parent
    / "prisma" / "migrations" / "20260811000001_seed_all_adf_tool_permissions" / "migration.sql"
)
...
def test_every_spec_name_exists_in_the_real_tool_registry():
    ...
    it. TOOL_REGISTRY is keyed by spec.name (the 66 distinct, LLM-facing names), not
    registry_tool_name, since the RBAC unification (2026-07-30, see tools/__init__.py's own
    docstring) — registry_tool_name now only identifies which of the 8 shared generic
    ...
    assert len(TOOL_REGISTRY) == 68


def test_generic_derived_specs_carry_resource_type_and_name_kwarg():
    rollback_dataset = next(spec for spec in SPECS if spec.name == "rollback_dataset_definition")
    assert rollback_dataset.registry_tool_name == "rollback_resource_definition"
    assert rollback_dataset.resource_type == "dataset"
    assert rollback_dataset.name_kwarg == "dataset_name"


def test_direct_specs_have_no_resource_type_or_name_kwarg_remapping():
    cancel_pipeline_run = next(spec for spec in SPECS if spec.name == "cancel_pipeline_run")
...
def test_get_resource_definition_raw_excludes_trigger():
    names = {spec.name for spec in SPECS if spec.registry_tool_name == "get_resource_definition_raw"}
    assert "get_trigger_definition_raw" not in names
    assert names == {
        "get_pipeline_definition_raw", "get_dataset_definition_raw",
        "get_linked_service_definition_raw", "get_data_flow_definition_raw",
        "get_global_parameter_definition_raw",
    }


def test_back_and_forward_descriptions_are_self_contained_not_cross_referencing():
    """Unlike claude-desktop's static full-tool-list MCP server, retrieval may select
    back_dataset_definition without also selecting back_pipeline_definition in the same turn -
    a "see back_pipeline_definition for the full behavior" cross-reference would then point at
    a tool the model can't see. Every kind's back_*/forward_* description must be fully
    self-contained instead."""
    for spec in SPECS:
        if spec.registry_tool_name in ("back_resource_definition", "forward_resource_definition"):
            assert "see " not in spec.description.lower() or "for the full behavior" not in spec.description.lower()
...
def test_every_spec_has_a_seeded_rbac_permissions_row():
    ...
    Parses the seed migration's literal tool_name values rather than re-deriving them, so this
    fails loudly if a new tool is ever added to SPECS without a matching seed row."""
    sql = _RBAC_SEED_MIGRATION.read_text()
    seeded_names = set(re.findall(r"^\s*\('([a-z_]+)', (?:true|false), (?:true|false), 'adf'\),?$", sql, re.MULTILINE))

    spec_names = {spec.name for spec in SPECS}
    assert seeded_names == spec_names, (
        f"missing from seed: {spec_names - seeded_names}; "
        f"seeded but no longer a real spec: {seeded_names - spec_names}"
    )
...
def test_retrieval_finds_the_expected_tool_for_representative_messages():
    ...
    cases = [
        ("Can you roll back the Foo dataset to its previous version?", "rollback_dataset_definition"),
    ...
    for message, expected in cases:
        selected = retrieve_relevant_tools(message, SPECS, index, top_k=8, always_include=always_include)
        selected_names = [spec.name for spec in selected]
        assert expected in selected_names, f"{expected!r} not retrieved for {message!r}: got {selected_names}"
