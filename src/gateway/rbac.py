...
async def set_platform_context(db, platform: str) -> None:
    """Sets the session GUC the rbac_permissions RLS policy (migration 20260804000002) checks
    against — set_config(..., true) is Postgres's parameter-bindable equivalent of `SET LOCAL`
    (plain SET doesn't accept bind parameters, only a literal, so this is the safe way to pass
    `platform` through without hand-building/escaping SQL text). Transaction-scoped, so it
    clears itself at the end of this session's transaction.

    Best-effort, deliberately swallows failures: sqlite (the test fixture) has no set_config at
    all, and some hand-rolled test doubles don't accept a params dict — neither should block
    the actual enforcement (_check_permission), which runs regardless. On real Postgres this
    is expected to always succeed (set_config needs no special privilege); a genuine failure
    there would just mean the RLS layer silently isn't scoped for that one call, not that
    _check_permission's own tool_name-based check stops running.
    """
    try:
        await db.execute(text("SELECT set_config('app.current_platform', :platform, true)"), {"platform": platform})
    except Exception:
        logger.debug("set_platform_context: skipped (unsupported by this session)", exc_info=True)
...
class RBACGateway:
    def __init__(
        ...
        """
        `investigation_id` is used for AuditLog rows (nullable since 2026-07-27 — ad-hoc chat
        tool calls have no FailureEvent to point at, so `None` is a real, expected value here,
        not a missing one).

        `thread_id` is passed explicitly (2026-08-06 fix) rather than derived from anything
        else — every failure-triggered chat call needs its own real thread_id on the AuditLog
        row, same as an ad-hoc call.
        """
        self._db = db
    ...
    async def call(
        ...
        # mechanism (FunctionTool(needs_approval=...), see mcp_servers/adf/tool_search_tool.py's build_chat_tools)
        # before this is ever called for a chat-agent tool; this check is the independent,
        # approval-mechanism-agnostic gate (defense in depth — see the "Expose all ADF tools"
        # plan's decision on keeping both RBAC layers).
        #
        # _check_permission is platform-scoped directly (2026-08-06 fix) rather than relying
        # solely on the rbac_permissions RLS policy (migration 20260804000002), which is a
        # documented no-op since the app connects as a Postgres superuser (unconditionally
        # bypasses RLS) — without this, a tool_name collision between two platforms would let
        # this call read the wrong platform's allowed/requires_consent row. set_platform_context
        # stays too, as the second, DB-level layer for once the connection role is fixed.
        ...
        await self._log(event_type, pipeline_id, project, platform, user_id, {"tool": tool_name, "arguments": arguments})
        if not allowed:
    ...
    async def _enrich(self, arguments: dict, project: str) -> dict:
        ...
        # it is a local AES operation, no network I/O, so there's nothing here worth caching
        # (the expensive part, the Azure SDK client itself, is cached separately — see
        # mcp_servers/adf/client_cache.py).
    ...
    async def _dispatch(self, tool_name: str, arguments: dict, project: str) -> dict:
        ...
        try:
            ...
            # own duplicate-call handling (tool_search_tool.py) is what decides whether to try
            # again, now against a freshly-built client.
            client_cache.invalidate(
                arguments["tenant_id"], arguments["client_id"],
                arguments["client_secret"], arguments["subscription_id"],
            )
            raise
...
async def call_tool(
    ...
    *,
    ...
    Runs RBACGateway in-process — radar is deployed as a single FastAPI service (2026-08-06:
    the standalone tool-exec-service split was scoped out, see git history for the removed
    server.py/tool_exec_auth.py)."""
    gateway = RBACGateway(
        db=db,
        infra_params=infra_params_dict, investigation_id=investigation_id, thread_id=thread_id,
    )
    return await gateway.call(
        tool_name=tool_name, arguments=arguments, user_id=user_id,
        pipeline_id=pipeline_id, project=project, platform=platform,
    )
