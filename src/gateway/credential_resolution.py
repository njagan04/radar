...
public."Credential" table directly, on every RBACGateway tool call. Decryption is a local
AES operation (no network I/O); RBACGateway already holds an open DB session for the same
call, so the secret is resolved fresh, per call, straight from that session.

tenant_id/client_id/subscription_id/resource_group/factory_name are not resolved here —
they're plain (non-secret) columns on RADAR's own `credentials` table, sourced via
...
async def resolve_client_secret(db: AsyncSession, project: str) -> str:
    """Cross-schema read of WatchTower's own Integrations credential store. Matched on
    projectName + service name='adf' since a project could have Credential rows for other
    services too. Takes the caller's already-open AsyncSession directly (not a factory) —
    this is a plain read alongside whatever else that session is already doing in the same
    request."""
