...
convention); Depends() is used only for get_current_user_email, so there's exactly one new
...
from chat.deps import get_current_user_email, get_current_user_id
...
def _thread_out(t) -> dict:
    ...
    # silently, permanently blocked from new messages (prepare_message_send 409s while this
    # is set — there was previously no way to see *why* short of reading server logs).
    pending = t.pending_tool_approval
    return {
        "thread_id": t.thread_id, "project": t.project, "investigation_id": t.investigation_id,
        "title": t.title, "claimed_by_user_id": t.claimed_by_user_id,
...
async def list_notifications(request: Request, project: str, user_email: str = Depends(get_current_user_email)):
    async with request.app.state.db_factory() as db:
        failure_events = await service.list_notifications(db, user_email, project)
        return {"notifications": [_notification_out(f) for f in failure_events]}


@router.post("/notifications/{investigation_id}/seen")
async def mark_notification_seen(request: Request, investigation_id: str, user_email: str = Depends(get_current_user_email)):
    async with request.app.state.db_factory() as db:
        failure_event = await service.mark_notification_seen(db, user_email, investigation_id)
        return {"notification": _notification_out(failure_event)}


@router.get("/notifications/{investigation_id}")
async def get_notification(request: Request, investigation_id: str, user_email: str = Depends(get_current_user_email)):
    async with request.app.state.db_factory() as db:
        failure_event = await service.get_pending_notification(db, user_email, investigation_id)
...
@router.get("/threads")
async def list_threads(request: Request, project: str, user_email: str = Depends(get_current_user_email)):
    async with request.app.state.db_factory() as db:
        threads = await service.list_threads(db, user_email, project)
        return {"threads": [_thread_out(t) for t in threads]}


@router.post("/threads")
async def create_thread(request: Request, body: CreateThreadRequest, user_email: str = Depends(get_current_user_email)):
    async with request.app.state.db_factory() as db:
        thread = await service.create_ad_hoc_thread(db, user_email, body.project, body.investigation_id)
        return _thread_out(thread)


@router.get("/threads/{thread_id}")
async def get_thread(
    request: Request, thread_id: str, before_id: int | None = None, limit: int = 30,
    user_email: str = Depends(get_current_user_email),
):
    async with request.app.state.db_factory() as db:
        thread, messages, has_more = await service.get_thread_with_messages(db, user_email, thread_id, before_id, limit)
        return {
...
async def get_thread_stats(request: Request, thread_id: str, user_email: str = Depends(get_current_user_email)):
    async with request.app.state.db_factory() as db:
        stats = await service.get_thread_stats(db, user_email, thread_id)
        notification_created_at = stats["notification_created_at"]
        return {
            **stats,
            "notification_created_at": notification_created_at.isoformat() if notification_created_at else None,
        }


@router.get("/admin/analytics")
async def get_admin_analytics(request: Request, user_email: str = Depends(get_current_user_email)):
    async with request.app.state.db_factory() as db:
        return await service.get_admin_analytics(db, user_email)


@router.post("/threads/{thread_id}/claim")
async def claim_thread(
    request: Request, thread_id: str,
    user_email: str = Depends(get_current_user_email), user_id: str = Depends(get_current_user_id),
):
    async with request.app.state.db_factory() as db:
        thread = await service.claim_thread(db, user_email, user_id, thread_id)
        return _thread_out(thread)
...
async def rename_thread(
    ...
    async with request.app.state.db_factory() as db:
        thread = await service.rename_thread(db, user_email, user_id, thread_id, body.title)
        return _thread_out(thread)
...
async def delete_thread(
    ...
    async with request.app.state.db_factory() as db:
        await service.delete_thread(db, user_email, user_id, thread_id)
        return {"deleted": True}
...
async def post_message(
    request: Request, response: Response, thread_id: str, body: MessageRequest,
    user_email: str = Depends(get_current_user_email), user_id: str = Depends(get_current_user_id),
):
    db_factory = request.app.state.db_factory
    async with db_factory() as db:
        outcome = await service.post_message(db, db_factory, request.app.state.redis, user_email, user_id, thread_id, body.content)
        return _outcome_response(response, outcome)


@router.post("/threads/{thread_id}/tool-approvals/resolve")
async def resolve_tool_approval(
    request: Request, response: Response, thread_id: str, body: ToolApprovalResolveRequest,
    user_email: str = Depends(get_current_user_email), user_id: str = Depends(get_current_user_id),
):
    db_factory = request.app.state.db_factory
    async with db_factory() as db:
        outcome = await service.resolve_tool_approval(
            db, db_factory, request.app.state.redis, user_email, user_id, thread_id,
            body.decision, body.tool_call_id, body.rejection_message,
        )
        return _outcome_response(response, outcome)
...
async def post_message_stream(
    request: Request, thread_id: str, body: MessageRequest,
    user_email: str = Depends(get_current_user_email), user_id: str = Depends(get_current_user_id),
):
    ...
    db = db_factory()
    try:
        thread, state, history = await service.prepare_message_send(db, db_factory, redis, user_email, user_id, thread_id, body.content)
    except Exception:
        await db.close()
        raise
    ...
    async def event_stream():
        try:
            async for event in service.stream_agent_reply_for_message(
                db, db_factory, redis, thread, state, history, body.content, user_id, thread_id,
            ):
                yield _sse(event)
        finally:
            await db.close()
...
async def resolve_tool_approval_stream(
    request: Request, thread_id: str, body: ToolApprovalResolveRequest,
    user_email: str = Depends(get_current_user_email), user_id: str = Depends(get_current_user_id),
):
    ...
    try:
        prepared = await service.prepare_tool_approval_resolution(
            db, db_factory, redis, user_email, user_id, thread_id, body.decision, body.tool_call_id, body.rejection_message,
        )
    except Exception:
        await db.close()
        raise
    ...
    if isinstance(prepared, service.PostMessageOutcome):
        await db.close()
        return JSONResponse({"status": prepared.kind, "detail": prepared.detail}, status_code=410)
    ...
    async def event_stream():
        try:
            async for event in service.stream_agent_reply_for_resume(
                db, db_factory, redis, thread, state, pending,
                body.decision, body.tool_call_id, body.rejection_message, user_id, thread_id,
            ):
                yield _sse(event)
        finally:
            await db.close()
...
async def stop_chat_stream(request: Request, thread_id: str, user_email: str = Depends(get_current_user_email)):
...
async def set_message_feedback(
    request: Request, message_id: int, body: FeedbackRequest,
    user_email: str = Depends(get_current_user_email), user_id: str = Depends(get_current_user_id),
):
    if body.rating not in ("up", "down"):
        raise HTTPException(status_code=400, detail="rating must be 'up' or 'down'")
    async with request.app.state.db_factory() as db:
        await service.set_message_feedback(db, user_email, user_id, message_id, body.rating)
    return {"status": "ok"}


@router.delete("/messages/{message_id}/feedback")
async def delete_message_feedback(
    request: Request, message_id: int,
    user_email: str = Depends(get_current_user_email), user_id: str = Depends(get_current_user_id),
):
    async with request.app.state.db_factory() as db:
        await service.set_message_feedback(db, user_email, user_id, message_id, None)
    return {"status": "ok"}
